...
<div>
 <input type="text" name="itext" id="myInput4" class="form-control" placeholder="Search">
 <table  id='table' class="table table-bordered table-hover table-striped" style="table-layout: fixed">  
  <thead>
  <tr>
    <th scope="cols">중단코드</th>
    <th scope="cols">중단품목명</th>
  </tr>
  </thead>
  <tbody id="myTable4">
  <?
    $stmt = $con->prepare("select * from iteminfo where usingY='N' ");
    $stmt->execute();
