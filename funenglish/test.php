...
function modify_info(){
     ...
     var ab =document.querySelectorAll('[id*="field-data-"]');
     var inputdata =  document.getElementById('datainput').value.split('/');
	 if (ab.length !== inputdata.length)
	 { 
		 alert('노드수가 맞지 않습니다.확인하여 주세요 ');

	 }
	 else{
	    
	     for (var i=0;i<ab.length;i++){ 
           ab.item(i).getElementsByClassName('selection_content')[0].innerHTML=inputdata[i];	
		 }
