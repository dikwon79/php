...
<html>
	<head>
		...
		function append_list(){
            ...
            var optioninfo = document.getElementById('option_info').value;

			$.post("iteminfo_append.php",{start:start, list:list, company:com, option_info : optioninfo , search:search},function(data){
       ...
       <section id="main">
					...
					<div class="col-md-10 my-2">
                     <? 
								...
								$optioninfo = isset($_POST['option_info']) ? $_POST['option_info'] : 'Y';
                     ...
                     ?>
								...
								<div class="title">조회>품목정보</div>
								<div id="navitem">
								<form class="navbar" role="search" method="post" action="iteminfo.php">
								   ...
								   <select name="option_info" id="option_info" class="form-control">
										<option value="all" <? if($optioninfo == 'all') echo "selected"; ?>>ALL</option>
										<option value="Y" <? if($optioninfo == 'Y') echo "selected"; ?>>Y</option>
										<option value="N" <? if($optioninfo == 'N') echo "selected"; ?>>N</option>
	
								   </select>
                                   </div>
								   <div class="col-sm-3">
								   
								   <input type="text" name="companyname"  id="companyname" class="form-control" placeholder="업체명" value="<? if($searchtext1) echo $searchtext1  ?>"></div>
								   ...
								   <div class="col-sm-4"><button type="submit" class="btn btn-default">검색</button>&nbsp;
								   <button type="button" class="btn btn-default" onclick="fnExcelReport('table', '품목정보')">엑셀저장</button>
								   <label class="fontlabel">업체품목보이기</label><input class="largecheck" type="checkbox" onclick="hide()">
								   </div>
									...
									<div class="row">
										...
										<table id ="table" class="table table-bordered table-hover table-striped" style="table-layout: fixed">  
											...
											<tr>  
												...
												<th width='80px'>출고코드</th>
												<th width='80px'>재고코드</th>
												<th width='300px'>품목명</th>
												<th class="noshow" width='400px'>업체품목</th>
																		
												<th width='80px'>렉번호</th> 
												<th width='30px'>입수</th>  
												<th width='30px'>출고수</th>  
												<th width='40px'>수도권블랙</th>
												<th width='40px'>품목불랙</th>  
												<th width='40px'>라벨발행유무</th>  
