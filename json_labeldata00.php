<?php
	...
	foreach ($array['maindata'] as $row) {
		...
		$sql = "select * from labelcjimsi where idate='$processdate' and chasu='$inputchasu' group by geocode";
		
        $result = mysqli_query($connect,$sql);
		
		$conditional =  "and (geocode='' or ";
 		while($row=mysqli_fetch_assoc($result)){
        ...
        $sql =" SELECT idate,blk,worktime,chasu,
           ...
           WHERE chasu ='$inputrchasu1' AND idate='$inputdate' ".$conditional.")x
		...
		{           
	               ...
	               $sql ="SET @maxID :='$smx'; ";
	               mysqli_query($connect,$sql);
				   $sql ="insert into labelmain (id,idate,blk,worktime,chasu, type, center,geocode, geoname, itemcode, itemname, customer, surang1,surang2,chaivalue,unit,bar1,bar2,mcode,deaddate,mcondition, cha2,special,groupnum) 
				   ...
				   (CASE when chaivalue >0 AND surang1=0  then '신규' when chaivalue >0 AND surang1!=0 then '*증가'			
						   ...
						   when chaivalue =0 and !(surang1=0 AND surang2=0) AND (bar1 !=bar2) then 	'*바코드변경'	              
						   when chaivalue =0 and !(surang1=0 AND surang2=0) AND (cha1 != cha2) then '*차량번호변경' END) AS 'kind',
				   center, geocode,geoname,itemcode,itemname,customer,surang1,surang2,chaivalue,unit2,bar1,bar2,mcode,deaddate,mcondition,cha2,special,groupnum 
				   ...
				   sum(if (cancel2='N',su2,'0'))-sum(if (cancel1='N',su1,'0')) AS 'chaivalue',MAX(if (cancel1='N',barcode1,'0')) AS 'bar1' , MAX(if (cancel2='N',barcode2,'0')) AS 'bar2', 
				   mcode,deaddate,mcondition, cha1, cha2,special,groupnum FROM (SELECT  u.idate,u.blk,u.worktime,u.chasu, u.center,u.geocode,u.geoname, u.itemcode,u.itemname,u.customer, x.cancelY as 'cancel1',x.unit AS 'unit1',x.surang AS 'su1' ,
				   u.cancelY as 'cancel2', u.unit as 'unit2' , u.surang AS 'su2' ,u.surang-x.surang as 'chai',
				   x.barcode AS 'barcode1', u.barcode AS 'barcode2', u.mcode,u.deaddate,u.mcondition,x.deliver as 'cha1', u.deliver as 'cha2', u.special,u.groupnum 
				   FROM (SELECT a.itemcode, a.itemname , a.customer ,a.cancelY,a.unit, a.surang, a.barcode, a.deliver,a.groupnum FROM labelcj a 
				   WHERE chasu ='$inputrchasu1' AND idate='$inputdate' ".$conditional.")x
				   right JOIN (SELECT b.idate,b.blk,b.worktime,b.chasu, b.center,b.geocode,b.geoname, b.itemcode, b.itemname , b.customer ,b.cancelY,b.unit, b.surang, b.barcode ,
				   b.mcode, b.deaddate ,b.mcondition, b.deliver, b.special,b.groupnum FROM labelcjimsi b WHERE chasu =' $inputrchasu2' AND idate='$inputdate')u 
				   ON x.barcode = u.barcode)T GROUP BY center,itemcode,customer,unit2 HAVING chaivalue !=0 OR bar1 <> bar2 OR cha1 <> cha2)S WHERE !(surang1 =0 AND surang2=0)";
			   ...
			   //비교용 labelcj에 비교 원본을 넣어준다. 
				  ...
				  $sql = "insert into labelcj(idate, blk, worktime, chasu, differ, center, geocode,geoname,itemcode, itemname ,customer,deadtype,unit,surang,deliver,delivergubun,mcode ,mcondition ,special ,deaddate ,barcode,orderY ,cancelY,emergenY,sorterY ,groupnum )
				SELECT   u.idate,u.blk,u.worktime,$inputchasu,u.differ,u.center,u.geocode,u.geoname, u.itemcode,u.itemname,u.customer,u.deadtype,u.unit,u.surang,u.deliver,u.delivergubun,u.mcode,u.mcondition,u.special,u.deaddate,u.barcode,
				u.orderY,u.cancelY,u.emergenY,u.sorterY,u.groupnum FROM (SELECT * FROM labelcj a 
				   ...
				   UNION ALL
				   SELECT   x.idate,x.blk,x.worktime,$inputchasu,x.differ,x.center,x.geocode,x.geoname, x.itemcode,x.itemname,x.customer,x.deadtype,x.unit,x.surang,x.deliver,x.delivergubun,x.mcode,x.mcondition,x.special,x.deaddate,x.barcode,
		...
		else
		{
	            ...
	            $sql = "delete from labelcjimsi where idate='$processdate' and chasu='$inputchasu'";
                mysqli_query($connect,$sql);
				$connect->close();
				echo "변경분이 없습니다.";
		}
