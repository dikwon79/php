<?php
    ...
    // 각 업체별 차수가 다르므로...... 진정한 차수를 구해준다.
	$sql = "SELECT idate, geocode , MAX(realchasu) as diff FROM labelcjimsi where  geocode='$geocode' and differ='$stringchai' and idate='$processdate' "; 
	$result = mysqli_query($connect,$sql);
    $row = mysqli_fetch_assoc($result);

	if ($row['diff']) {$realchasu = $row['diff']+1;}
	else {$realchasu =1;}
	...
	foreach ($array['maindata'] as $row) {
			...
			$sql = "insert into labelcjimsi(idate, blk, geocode, geoname, worktime, chasu, realchasu ,differ, center, itemcode, itemname ,customer,deadtype,unit,surang,deliver,delivergubun,mcode ,mcondition ,special ,deaddate ,barcode,orderY ,cancelY,emergenY,sorterY ,groupnum ) VALUES('".addslashes($array['daten'])."','".addslashes($blk)."','".addslashes($geocode)."','".addslashes($geoname)."','".addslashes($time)."','".addslashes($array['chasu'])."','".addslashes($realchasu)."','".addslashes($stringchai)."','".addslashes($str[0])."','".addslashes($str[1])."','".addslashes($str[2])."','".addslashes($str[3])."','".addslashes($str[4])."',
		...
		$sql = "SELECT MAX(realchasu) as 'geochasu' FROM labelcjimsi WHERE chasu='$inputchasu' AND idate='$inputdate' and geocode='$geocode';";
		$result = mysqli_query($connect,$sql);
		$count = mysqli_num_rows($result);
        //echo $sql;
		if ($count > 0) {
		while($row = mysqli_fetch_assoc($result)) {
			  ...
			  $sql ="insert into labelmain (id,idate,blk,worktime,chasu,geocode,geoname, type, center,itemcode, itemname, customer, surang1,surang2,chaivalue,unit,bar1,bar2,mcode,deaddate,mcondition, cha2,special,groupnum) 
	       SELECT @maxID:=@maxID+1,idate,blk,worktime,chasu,geocode,geoname, 
           ...
           mcode,deaddate,geocode,geoname,mcondition, cha1, cha2,special,groupnum FROM (SELECT  u.idate,u.blk,u.worktime,u.chasu, u.center, u.itemcode,u.itemname,u.customer, x.cancelY as 'cancel1',x.unit AS 'unit1',x.surang AS 'su1' ,
           u.cancelY as 'cancel2', u.unit as 'unit2' , u.surang AS 'su2' ,u.surang-x.surang as 'chai',
           x.barcode AS 'barcode1', u.barcode AS 'barcode2', u.mcode,u.deaddate,u.geocode,u.geoname,u.mcondition,x.deliver as 'cha1', u.deliver as 'cha2', u.special,u.groupnum 
           FROM (SELECT a.itemcode, a.itemname , a.customer ,a.cancelY,a.unit, a.surang, a.barcode, a.deliver,a.groupnum FROM labelcj a 
           WHERE realchasu ='$inputrchasu1' AND idate='$inputdate' and geocode ='$inputgeocode')x
           ...
           b.mcode, b.deaddate ,b.geocode,b.geoname,b.mcondition, b.deliver, b.special,b.groupnum FROM labelcjimsi b WHERE realchasu =' $inputrchasu2' AND idate='$inputdate' and geocode ='$inputgeocode')u 
       ...
       //비교용 labelcj에 비교 원본을 넣어준다. 
        ...
        $sql = "insert into labelcj(idate, blk, worktime,geocode,geoname, chasu, realchasu ,differ, center, itemcode, itemname ,customer,deadtype,unit,surang,deliver,delivergubun,mcode ,mcondition ,special ,deaddate ,barcode,orderY ,cancelY,emergenY,sorterY ,groupnum )
		SELECT   u.idate,u.blk,u.worktime,u.geocode,u.geoname,$inputchasu,$inputrchasu2,u.differ,u.center, u.itemcode,u.itemname,u.customer,u.deadtype,u.unit,u.surang,u.deliver,u.delivergubun,u.mcode,u.mcondition,u.special,u.deaddate,u.barcode,
        u.orderY,u.cancelY,u.emergenY,u.sorterY,u.groupnum FROM (SELECT * FROM labelcj a 
           WHERE realchasu ='$inputrchasu1' AND idate='$inputdate' and geocode ='$inputgeocode')x
           right JOIN (SELECT * FROM labelcjimsi b WHERE realchasu ='$inputrchasu2' AND idate='$inputdate' and geocode ='$inputgeocode'  )u 
           ON x.barcode = u.barcode
           UNION ALL
           SELECT   x.idate,x.blk,x.worktime,x.geocode,x.geoname,$inputchasu,$inputrchasu2,x.differ,x.center, x.itemcode,x.itemname,x.customer,x.deadtype,x.unit,x.surang,x.deliver,x.delivergubun+9,x.mcode,x.mcondition,x.special,x.deaddate,x.barcode,
           x.orderY,x.cancelY,x.emergenY,x.sorterY,x.groupnum FROM (SELECT * FROM labelcj a 
           WHERE realchasu ='$inputrchasu1' AND idate='$inputdate' and geocode ='$inputgeocode')x 
           left JOIN (SELECT * FROM labelcjimsi b WHERE realchasu ='$inputrchasu2' AND idate='$inputdate' and geocode ='$inputgeocode')u   ON x.barcode = u.barcode WHERE  u.surang IS NULL";
        mysqli_query($connect,$sql);
