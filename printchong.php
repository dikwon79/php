...
else
{
    ...
    */
	$sql="insert into labelmain (id,worker,idate,blk,worktime, companyname, chasu, type, center,geocode, geoname, itemcode, mappingcode, itemname, customer, surang1,surang2,chaivalue,unit,bar1,bar2,mcode,deaddate,mcondition, cha2,special,cancelY,groupnum) 
	SELECT @maxID:=@maxID+1,worker,idate,blk,worktime,companyname,chasu,
	(CASE when chaivalue >0 AND surang1=0  then '신규' when chaivalue >0 AND surang1!=0 then '*증가'			
	when chaivalue <0 and surang2=0  then '*취소'
	when chaivalue <0 and surang2!=0 then '*감소'	           
	when chaivalue =0 and !(surang1=0 AND surang2=0) AND (bar1 !=bar2) then 	'*바코드변경'	              
	when chaivalue =0 and !(surang1=0 AND surang2=0) AND (cha1 != cha2) then '*차량번호변경' END) AS 'kind',
	center,geocode,geoname,itemcode,mappingcode,itemname,customer,surang1,surang2,chaivalue,unit2,barcode1,barcode2,mcode,deaddate,mcondition,cha2,special,cancel2,groupnum 
	...
	sum(if (cancel2='N',su2,'0'))-sum(if (cancel1='N',su1,'0')) AS 'chaivalue',MAX(if (cancel1='N',barcode1,'0')) AS 'bar1' , MAX(if (cancel2='N',barcode2,'0')) AS 'bar2'
	,barcode1,barcode2,mcode,deaddate,mcondition, cha1, cha2,special,cancel2,groupnum FROM (SELECT p.worker,p.idate,p.blk,p.worktime,p.companyname, p.chasu, p.center,p.geocode,p.geoname, p.itemcode,q.mappingcode,p.itemname,p.customer, p.cancel1,p.unit1,p.su1,
	p.cancel2, p.unit2 , p.su2 ,p.chai,p.barcode1, p.barcode2, p.mcode,p.deaddate,p.mcondition,p.cha1,p.cha2, p.special,p.groupnum from(SELECT  u.worker,u.idate,u.blk,u.worktime,u.companyname, u.chasu, u.center,u.geocode,u.geoname, u.itemcode,u.itemname,u.customer, x.cancelY as 'cancel1',x.unit AS 'unit1',x.surang AS 'su1' ,
	u.cancelY as 'cancel2', u.unit as 'unit2' , u.surang AS 'su2' ,u.surang-x.surang as 'chai',
	x.barcode AS 'barcode1', u.barcode AS 'barcode2', u.mcode,u.deaddate,u.mcondition,x.deliver as 'cha1', u.deliver as 'cha2', u.special,u.groupnum 
	FROM (
	SELECT itemcode,itemname,customer,cancelY,unit,if(cancelY='Y',surang1,surang2) AS 'surang', if(cancelY='Y',bar1,bar2) AS 'barcode',cha2 AS 'deliver', groupnum from(select * from labelmain where idate='$inputdate' and (substr(bar2,1,18), chasu) in (select substr(bar2,1,18), max(chasu) as chasu from labelmain where chasu < '$inputchasu' and companyname='cj' and worker='".$_SESSION['user_id']."' group by substr(bar2,1,18))
	...
	b.cancelY,b.unit, sum(b.surang) as 'surang', MAX(concat(SUBSTR(b.barcode,1,18),SUBSTR(b.barcode,24,5))) AS 'barcode',b.mcode, b.deaddate ,b.mcondition, b.deliver, b.special,b.groupnum FROM labelcjimsi b 
	WHERE chasu ='$inputrchasu2' AND idate='$inputdate' and companyname='cj' and worker='".$_SESSION['user_id']."' GROUP BY substr(b.barcode,1,18),center,itemcode,customer)u 
...
if ($chongtype=='1'){
        ...
        $sql="SELECT T.printgubun,T.stype as 'type',T.printinfo as 'centername', T.mapcode,T.itemcode,T.item,T.customer,T.susu,T.unit as 'danwi',CONCAT (if (T.ipsu is not NULL,(T.susu div T.ipsu),0),' / ',if (T.ipsu is not NULL,(T.susu mod T.ipsu),T.susu)) as 'PT' ,T.mcondition,T.deliver as 'cha2',T.labelcount,T.lek 
FROM (SELECT b.printgubun,b.ipsu,Z.stype,Z.printinfo,Z.center, if(b.mappingcode IS NOT NULL,b.mappingcode,Z.itemcode) AS mapcode,Z.itemcode,if(b.itemname IS NULL,CONCAT('신규)'
,Z.itemname),b.itemname)AS 'item',Z.customer,Z.cancelY,Z.be4 as'susu',Z.unit,Z.barcode ,Z.mcondition,Z.deliver,Z.labelcount,b.lek FROM (SELECT '*취소' as stype, a.printinfo, S.center, S.itemcode, S.itemname, S.customer, S.cancelY,S.surang1 as be4,S.unit,S.bar1 as barcode,S.mcondition,S.cha2 as deliver,SUBSTR(bar1,19,5) as labelcount  from(SELECT * FROM labelmain WHERE (TYPE!='신규') 
and chasu='$chasu' AND idate= '$processdate' and companyname='cj' and worker='".$_SESSION['user_id']."'
ORDER BY companyname ASC,chasu asc,TYPE ASC)S LEFT JOIN print_info a ON 
 S.center = concat(a.center,' / ',a.centername))Z LEFT JOIN iteminfo b ON Z.itemcode = b.itemcode

UNION ALL

SELECT if(substring(b.lek,1,1)='H',concat(b.printgubun,'냉동'),if(SUBSTRING(b.lek,1,1)='I',CONCAT(b.printgubun,'냉동'),b.printgubun)),b.ipsu,Z.stype,Z.printinfo,Z.center, if(b.mappingcode IS NOT NULL,b.mappingcode,Z.itemcode) AS mapcode,Z.itemcode,if(b.itemname IS NULL,CONCAT('신규)'
,Z.itemname),b.itemname)AS 'item',Z.customer,Z.cancelY,
sum(CASE when trim(Z.unit) = 'BOX' then Z.af4*b.ipsu when TRIM(Z.unit)='EA' then Z.af4 else Z.af4 END) AS susu,Z.unit,Z.barcode ,Z.mcondition,Z.deliver,Z.labelcount,b.lek FROM (SELECT Y.stype,a.printinfo,Y.center, Y.itemcode,Y.itemname,Y.customer,Y.cancelY,Y.af4,Y.unit,Y.barcode ,Y.mcondition,Y.deliver,Y.labelcount  
FROM (SELECT '신규' AS 'stype',center, itemcode,itemname,customer,cancelY,af4,unit,barcode ,mcondition,deliver,labelcount FROM (SELECT SUM(t) AS 'joincheck',center, itemcode,itemname,customer,cancelY,sum(if(val0='b1',su,0)) AS 'be4' ,sum(if(val0='b2',su,0)) AS 'af4' ,unit,barcode ,mcondition,deliver,labelcount FROM(
   SELECT '0' AS 't', 'b1' AS val0, center, itemcode,itemname, customer,if(cancelY='N',surang,0) AS 'su', unit, barcode,mcondition,deliver,SUBSTR(barcode,19,5) AS 'labelcount',cancelY from labelcjimsi WHERE (companyname,idate, chasu,cancelY,barcode) in (select companyname,idate, MAX(chasu) as chasu ,cancelY,barcode from labelcjimsi where chasu < $chasu and companyname='cj' AND idate='$processdate' and worker='".$_SESSION['user_id']."' group by barcode)
   UNION ALL
	SELECT '1' AS 't','b2' as val0, center, itemcode,itemname, customer ,if(cancelY='N',surang,0) AS 'su',unit,barcode,mcondition,deliver,SUBSTR(barcode,19,5) AS 'labelcount',cancelY FROM labelcjimsi WHERE chasu='$chasu' AND idate= '$processdate' and companyname='cj' and worker='".$_SESSION['user_id']."'
)t1 GROUP BY barcode)t2 WHERE joincheck = '1' AND af4-be4 !=0 and af4!=0)Y LEFT JOIN print_info a ON Y.center = concat(a.center,' / ',a.centername))Z LEFT JOIN iteminfo b ON Z.itemcode = b.itemcode GROUP BY mapcode,Z.printinfo)T ORDER BY stype ASC, printgubun ASC , printinfo ASC, T.item asc, T.susu asc, T.customer
";
...
<body onbeforeunload="return myFunction()">
  <div id='control' class="control">
	...
	<?
				  ...
				  while($row=$stmt->fetch(PDO::FETCH_ASSOC))
				  {
					  ...
					  {  
						   ...
						   if (($check !=trim($type)) or ($center !=trim($centername)) or ($printgubuncheck !=trim($printgubun))) {
							   ...
							   if ($check=='신규'){ ?>
                              <tr><td colspan='7' style="text-align:center; font-weight: bold;"><? echo $center."  1총수량: ".$totalsurang?></td></tr>
							  <? } ?>
							  </tbody></table></div>
		                      </div> 
							   <?
						   ...
						   <div><h1><? if ($printgubun===null){ echo '품목미등록'.'-'.$chasu.'차작업';} 
						               else { echo $printgubun.'-'.$chasu.'차작업';} ?></h1></div>
                    ...
                    if ($heightvalue>910 or $heightvalue2>910){   
		...
		?>
        <tr><td colspan='7' style="text-align:center; font-weight: bold;"><? echo $centername."  총수량: ".$totalsurang?></td></tr>
