...
,Z.itemname),b.itemname)AS 'item',Z.customer,Z.cancelY,Z.be4 as'susu',Z.unit,Z.barcode ,Z.mcondition,Z.deliver,Z.labelcount,b.lek FROM (SELECT Y.stype,a.printinfo,Y.center, Y.itemcode,Y.itemname,Y.customer,Y.cancelY,Y.be4,Y.unit,Y.barcode ,Y.mcondition,Y.deliver,Y.labelcount  FROM (
    SELECT '*취소' AS 'stype',center, itemcode,itemname,customer,cancelY,be4,unit,barcode ,mcondition,deliver,labelcount 
...
FROM(
   ...
   UNION ALL
	SELECT '1' AS 't','b2' as val0, center, itemcode,itemname, customer ,if(cancelY='N',surang,0) AS 'su',unit,barcode,mcondition,deliver,SUBSTR(barcode,19,5) AS 'labelcount',cancelY FROM labelcjimsi WHERE chasu='$chasu' AND idate= '$processdate' and worker='".$_SESSION['user_id']."'
...
sum(CASE when trim(Z.unit) = 'BOX' then Z.af4*b.ipsu when TRIM(Z.unit)='EA' then Z.af4 else Z.af4 END) AS susu
,Z.unit,Z.barcode ,Z.mcondition,Z.deliver,Z.labelcount,b.lek FROM (SELECT Y.stype,a.printinfo,Y.center, Y.itemcode,Y.itemname,Y.customer,Y.cancelY,Y.af4,Y.unit,Y.barcode ,Y.mcondition,Y.deliver,Y.labelcount  
