...
if ($chongtype=='1'){
        ...
        $sql="SELECT T.printgubun,T.stype AS 'type',T.printinfo AS 'centername', T.mapcode,T.itemcode,T.item,T.customer,T.susu,T.af4,T.chai,T.unit AS 'danwi', CONCAT (if (T.ipsu IS NOT NULL,(T.susu DIV T.ipsu),if(T.unit='BOX',T.susu,0)),' / ',if(T.ipsu IS NOT NULL,(T.susu MOD T.ipsu),if(T.unit='BOX',0,T.susu))) AS 'PT',T.mcondition,T.deliver AS 'cha2',T.labelcount,T.lek,T.prechasu,T.ipsu
FROM (
SELECT b.printgubun,b.ipsu,Z.stype,Z.printinfo,Z.center, if(b.mappingcode IS NOT NULL,b.mappingcode,Z.itemcode) AS mapcode,Z.itemcode,if(b.itemname IS NULL, CONCAT('신규)',Z.itemname),b.itemname) AS 'item',Z.customer,Z.cancelY,Z.susu,Z.af4,Z.chai,Z.unit,Z.barcode,Z.mcondition,Z.deliver,Z.labelcount,b.lek,Z.prechasu
FROM (
SELECT '*취소' AS 'sType', a.printinfo, S.center, S.itemcode, S.itemname, S.customer, S.cancelY,S.be4 AS susu,S.af4,S.chaivalue AS chai,S.unit,S.bar1 AS barcode,S.mcondition,S.cha2 AS deliver, SUBSTR(bar1,24,5) AS labelcount,
...
SELECT b.printgubun,b.ipsu,Z.stype,Z.printinfo,Z.center, if(b.mappingcode IS NOT NULL,b.mappingcode,Z.itemcode) AS mapcode,Z.itemcode,if(b.itemname IS NULL, CONCAT('신규)',Z.itemname),b.itemname) AS 'item',Z.customer,Z.cancelY, SUM(CASE WHEN TRIM(Z.unit) = 'BOX' THEN if(b.packsu,Z.af4*b.packsu,Z.af4)  WHEN TRIM(Z.unit)='EA' THEN Z.af4 ELSE Z.af4 END) AS susu,'','',Z.unit,Z.barcode,Z.mcondition,Z.deliver,Z.labelcount,b.lek,''
FROM (
...
ORDER BY stype ASC, printinfo ASC, printgubun ASC, T.lek asc, T.item ASC, T.susu ASC, T.customer ASC, T.labelcount ASC";
