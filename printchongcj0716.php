...
//from labelmain  where companyname='cj' AND idate='2020-09-23' group by substr(bar2,1,18)) 
   ...
   $sql ="insert into labelcjimsi(worker, idate, blk, worktime, geocode,geoname,buydan,companyname, chasu ,realchasu,differ,center, itemcode, itemname ,customer,deadtype,unit,surang,deliver,
	...
	SELECT worker,idate,blk,worktime,'','','',companyname,'$inputrchasu2',realchasu,differ,center,itemcode,itemname,customer,deadtype,unit,be4,cha2,delivergubun,mcode
	,mcondition,special,deaddate,bar1,'N','Y','N','N',groupnum,labelcount
	FROM (
	SELECT SUM(t) AS 'joincheck', worker,idate,blk,worktime,companyname, chasu,differ,realchasu,center,itemcode,itemname,customer,deadtype,deaddate, 
	MAX(if(val0='b1',special,'')) AS special,cancelY, MAX(if(val0='b1',1,0)) AS 'gubun1', MAX(if(val0='b2',1,0)) AS 'gubun2', 
	SUM(if(val0='b1',su,0)) AS 'be4', SUM(if(val0='b2',su,0)) AS 'af4', SUM(if(val0='b2',su,0))- SUM(if(val0='b1',su,0)) AS 'chaivalue',
	unit,barcode AS 'bar1',mcondition,deliver AS 'cha2',delivergubun, labelcount,mcode,groupnum
	FROM(
	SELECT '0' AS 't', 'b1' AS val0, worker,idate,blk,worktime,companyname, chasu, differ,realchasu,center,itemcode,itemname, customer,deadtype,deaddate,
	special,if(cancelY='N',surang,0) AS 'su', unit, barcode,mcondition,deliver, delivergubun, SUBSTR(barcode,24,5) AS 'labelcount',mcode,cancelY,
	groupnum
	FROM labelcjimsi
	...
	SELECT '1' AS 't','b2' AS val0, worker,idate,blk,worktime,companyname,chasu, differ,realchasu,center, itemcode,itemname, customer,deadtype,deaddate,
	special,if(cancelY='N',surang,0) AS 'su',unit,barcode,mcondition,deliver, delivergubun, SUBSTR(barcode,24,5) AS 'labelcount',mcode,cancelY,
	groupnum
	FROM labelcjimsi
	WHERE chasu='$chasu' AND idate= '$processdate' AND companyname='cj' AND worker='".$_SESSION['user_id']."')t1
	GROUP BY CONCAT(SUBSTR(barcode,1,18), SUBSTR(barcode,24,5)))W
	WHERE joincheck = 0";
			 ...
			 if ($stmt->rowCount() > 0)
			 {
				  ...
				  while($row=$stmt->fetch(PDO::FETCH_ASSOC))
				  {
					  ...
					  else
					  {
						  ...
						  <td style="font-size:14px;" NOWRAP><?=$printinfo?></td>
