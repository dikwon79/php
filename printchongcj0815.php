...
else
{
   ...
   $sql ="insert into labelcjimsi(worker, idate, blk, worktime, geocode,geoname,buydan,companyname, chasu ,realchasu,differ,center, itemcode, itemname ,customer,deadtype,unit,surang,deliver,
	...
	SELECT B.worker,B.idate,B.blk,B.worktime,'','','',B.companyname,'$inputrchasu2',B.realchasu,B.differ,B.center,B.itemcode,B.itemname,B.customer,B.deadtype,B.unit,B.be4,B.cha2,B.delivergubun,B.mcode,B.mcondition,B.special,B.deaddate,B.bar1,'N','Y','N','N',B.groupnum,B.labelcount
FROM(
...
WHERE (B.gubun1 ='1' AND B.gubun2 ='1') AND B.chaivalue !=0";
...
FROM labelcjimsi
WHERE (companyname,idate, chasu,cancelY) in (
SELECT companyname,idate, MAX(chasu) AS chasu,cancelY
FROM labelcjimsi
WHERE chasu < $chasu AND companyname='cj' AND idate='$processdate' AND worker='".$_SESSION['user_id']."'
GROUP BY CONCAT(SUBSTR(barcode,1,18), SUBSTR(barcode,24,5))) UNION ALL
