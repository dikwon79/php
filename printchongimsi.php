...
else
{
    ...
    */
	...
	$sql="insert into labelmain (id,worker,idate,blk,worktime, companyname, chasu, type, center,geocode, geoname, itemcode, mappingcode, itemname, customer, surang1,surang2,chaivalue,unit,bar1,bar2,mcode,deaddate,mcondition, cha2,special,cancelY,groupnum,TOT_PAGE) 
	SELECT @maxID:=@maxID+1,worker,idate,blk,worktime,companyname,chasu,
	(CASE when chaivalue >0 AND surang1=0  then '신규' when chaivalue >0 AND surang1!=0 then '*증가'			
	when chaivalue <0 and surang2=0  then '*취소'
	when chaivalue <0 and surang2!=0 then '*감소'	           
	when chaivalue =0 and !(surang1=0 AND surang2=0) AND (bar1 !=bar2) then 	'*바코드변경'	              
	when chaivalue =0 and !(surang1=0 AND surang2=0) AND (cha1 != cha2) then '*차량번호변경' END) AS 'kind',
	center,geocode,geoname,itemcode,mappingcode,itemname,customer,surang1,surang2,chaivalue,unit2,barcode1,barcode2,mcode,deaddate,mcondition,cha2,special,cancel2,groupnum,totalsu 
	...
	sum(if (cancel2='N',su2,'0'))-sum(if (cancel1='N',su1,'0')) AS 'chaivalue',MAX(if (cancel1='N',barcode1,'0')) AS 'bar1' , MAX(if (cancel2='N',barcode2,'0')) AS 'bar2'
	,barcode1,barcode2,mcode,deaddate,mcondition, cha1, cha2,special,cancel2,groupnum,sum(if(cancel2='N',labelpage,'0')) as 'totalsu' FROM (SELECT p.worker,p.idate,p.blk,p.worktime,p.companyname, p.chasu, p.center,p.geocode,p.geoname, p.itemcode,q.mappingcode,p.itemname,p.customer, p.cancel1,p.unit1,p.su1,
...
if ($chongtype=='1'){
        ...
        $sql="SELECT T.printgubun,T.stype AS 'type',T.printinfo AS 'centername', T.mapcode,T.itemcode,T.item,T.customer,T.susu,T.af4,T.chai,T.unit AS 'danwi', CONCAT (if (T.ipsu IS NOT NULL,(T.susu DIV T.ipsu),0),' / ',if (T.ipsu IS NOT NULL,(T.susu MOD T.ipsu),T.susu)) AS 'PT',T.mcondition,T.deliver AS 'cha2',T.labelcount,T.lek
FROM (
SELECT b.printgubun,b.ipsu,Z.stype,Z.printinfo,Z.center, if(b.mappingcode IS NOT NULL,b.mappingcode,Z.itemcode) AS mapcode,Z.itemcode,if(b.itemname IS NULL, CONCAT('신규)',Z.itemname),b.itemname) AS 'item',Z.customer,Z.cancelY,Z.susu,Z.af4,Z.chai,Z.unit,Z.barcode,Z.mcondition,Z.deliver,Z.labelcount,b.lek
FROM (
SELECT S.type AS 'sType', a.printinfo, S.center, S.itemcode, S.itemname, S.customer, S.cancelY,S.surang1 AS susu,S.surang2 AS af4,S.chaivalue AS chai,S.unit,S.bar1 AS barcode,S.mcondition,S.cha2 AS deliver, SUBSTR(bar1,19,5) AS labelcount
FROM(
SELECT *
FROM labelmain
WHERE (TYPE!='신규') AND chasu='$chasu' AND idate= '$processdate' AND companyname='cj' AND worker='".$_SESSION['user_id']."'
ORDER BY companyname ASC,chasu ASC, TYPE ASC)S
LEFT JOIN print_info a ON S.center = CONCAT(a.center,' / ',a.centername))Z
LEFT JOIN iteminfo b ON Z.itemcode = b.itemcode UNION ALL
SELECT if(SUBSTRING(b.lek,1,1)='H', CONCAT(b.printgubun,'냉동'),if(SUBSTRING(b.lek,1,1)='I', CONCAT(b.printgubun,'냉동'),b.printgubun)),b.ipsu,Z.stype,Z.printinfo,Z.center, if(b.mappingcode IS NOT NULL,b.mappingcode,Z.itemcode) AS mapcode,Z.itemcode,if(b.itemname IS NULL, CONCAT('신규)',Z.itemname),b.itemname) AS 'item',Z.customer,Z.cancelY,SUM(CASE WHEN TRIM(Z.unit) = 'BOX' THEN Z.af4*b.ipsu WHEN TRIM(Z.unit)='EA' THEN Z.af4 ELSE Z.af4 END) AS susu,'','',Z.unit,Z.barcode,Z.mcondition,Z.deliver,Z.labelcount,b.lek
FROM (
SELECT Y.stype,a.printinfo,Y.center, Y.itemcode,Y.itemname,Y.customer,Y.cancelY,Y.af4,Y.unit,Y.barcode,Y.mcondition,Y.deliver,Y.labelcount
FROM (
...
ORDER BY stype ASC, printgubun ASC, printinfo ASC, T.item ASC, T.susu ASC, T.customer";
