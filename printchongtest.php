...
function inventory(){
      ...
      const pagenation = document.querySelectorAll(".itmcode");
	  var i=0;
      while(i < pagenation.length){
      alert(pagenation[i].textContent);
	  i = i +1;
   }
...
FROM labelcjimsi
WHERE (chasu, itemcode, customer) IN(
SELECT MAX(chasu), itemcode,customer FROM labelcjimsi WHERE chasu < $chasu AND companyname='cj' AND idate='$processdate' AND worker='".$_SESSION['user_id']."' 
...
chaivalue AS 'su3',unit,cha2,mcondition,geocode,bar2,mcode FROM labelmain WHERE TYPE !='신규' and chasu='$chasu' AND idate='$processdate' and companyname='cj')c
 ...
 group by type,itemcode, centername,danwi )S  GROUP BY type,mapcode, centername ORDER BY TYPE ASC ,printgubun asc,centername ASC,item ASC";
  ...
  <div id='control' class="control">
	...
	<button type="button" class="btn btn-default" onclick="inventory()">재고출력</button>
  </div>
			 ...
			 if ($stmt->rowCount() > 0)
			 {
				  ...
				  while($row=$stmt->fetch(PDO::FETCH_ASSOC))
				  {
					  ...
					  {  
						 ...
						 ?>
					    <tr>
						<td width='10%' class="itmcode"><?=$itemcode?></td>
						<td width='10%'><?=$lek?></td>
