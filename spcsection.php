 <!-- 라벨 -->
<?
    $sql = "SELECT sum(substr(bar2,19,5)) as 'labelsu' from labelmain 
           WHERE TYPE!='*취소' AND (companyname,idate,substr(bar2,1,18),substr(bar2,19,5), chasu) in (select companyname,idate,substr(bar2,1,18),substr(bar2,19,5), max(chasu) as chasu 
           from labelmain  where companyname='spc' AND idate='$value' group by substr(bar2,1,18))";		
	$stmt = $con->prepare($sql);
	$stmt->execute();
	$row = $stmt->fetch();  
	$labelsu = $row['labelsu'];

	$sql = "SELECT count(*) as 'blacksu' FROM labelcjimsi WHERE chasu=(select Max(chasu) FROM labelcjimsi WHERE companyname= 'spc' and idate='$value' AND cancelY='N') and companyname= 'spc' and idate='$value' AND cancelY='N'";		
	$stmt = $con->prepare($sql);
	$stmt->execute();
	$row = $stmt->fetch();  
	$blacksu = $row['blacksu']-$labelsu;
...
<div id='billboard2' class="col-sm-6">블랙라벨:<?=$blacksu?></div>
</div>
 ...
 <div class="row">
	<table id="table" class="table table-bordered table-hover table-striped" style="border: 1px solid; border-collapse: collapse;">
		...
		<tr>  
			...
			<? for ($i=1 ;$i<$chasu;$i++) { ?>
			<th><button type="button" class="btn btn-primary" onclick="windowSize('printchongspc.php',<?=$i?>,'width=900,height=558')"><?=$i.'차';?></button>
			<a class="btn btn-warning" href="workdelete.php?idate='<?=$value?>'&chasu='<?=$i?>'&companyname='spc'" onclick="return confirm('<?php echo $i ?> 차수를 삭제할까요?')">
			Del</a></th>
		...
		$sql .=" ,y.worktime FROM (SELECT * FROM geoinfo WHERE tpl='Y' and labeltype like '%SPC%')x LEFT JOIN (SELECT x.blk,ifnull(a.geocode,'100') as 'geo',COUNT(x.chasu) as 'cha' , x.chasu,x.worktime FROM (SELECT blk,itemcode, b.chasu,b.worktime from labelmain b where companyname='spc' and idate='$value')x LEFT JOIN iteminfo a ON x.itemcode = a.itemcode GROUP BY a.georae,x.chasu)y ON x.geocode = y.geo GROUP BY x.geoname order by y.chasu desc,x.pid asc";
