 ...
 <div class="row">
	<table id="table" class="table table-bordered table-hover table-striped" style="border: 1px solid; border-collapse: collapse;">
		...
		<tr>  
			...
			<? for ($i=1 ;$i<$chasu;$i++) { ?>
			<th><button type="button" class="btn btn-primary" onclick="windowSize('printchongwelstory.php',<?=$i?>,'width=900,height=558')"><?=$i.'차';?></button>
		...
		$sql .=" ,y.worktime FROM (SELECT * FROM geoinfo WHERE tpl='Y' and labeltype like '%웰스토리%')x LEFT JOIN (SELECT x.blk,ifnull(a.geocode,'100') as 'geo',COUNT(x.chasu) as 'cha' , x.chasu,x.worktime FROM (SELECT blk,itemcode, b.chasu,b.worktime from labelmain b where companyname='welstory' and idate='$value')x LEFT JOIN iteminfo a ON x.itemcode = a.itemcode GROUP BY a.georae,x.chasu)y ON x.geocode = y.geo GROUP BY x.geoname order by y.chasu desc,x.pid asc";
